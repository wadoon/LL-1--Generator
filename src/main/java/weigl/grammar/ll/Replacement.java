...
public class Replacement implements Comparable<Replacement>
{
    ...
    public Replacement(String line)
    {
        ...
        // .split("((?<=[a-zA-Z])(?=[A-Z]))|((?<=[A-Z])(?=[a-z]))");
                        // split each UpperCase split each lowerCase if
                        // upperCase
                        .split("((?<=.)(?=[A-Z]))|((?<=[A-Z])(?=[^A-Z]))");
        tokens = new Token[split.length];
        for (int i = 0; i < tokens.length; i++)
        {
            if (Character.isUpperCase(split[i].charAt(0))) tokens[i] = new Token.NonTerminalSymbol(
                            split[i]);
            else tokens[i] = new Token.TerminalSymbol(split[i]);
        }
    }
    ...
    {
        if (o.tokens.length != tokens.length) return tokens.length - o.tokens.length;
    ...
    public boolean hasEpsilon()
    {
        for (Token token : tokens)
            if (token.equals(Token.EPSILON)) return true;
        return false;
    }
