...
{
    private Multimap<String, Replacement>        productionRules = TreeMultimap.create();
    ...
    private void parseLine(String line) throws GrammarParserException
    {
        ...
        if (semiColon < 0) throw new GrammarParserException("no semicolon in line");

        // rip off the name
        String name = line.substring(0, semiColon).toUpperCase();

        if (name.isEmpty()) throw new GrammarParserException("non-terminal name is empty.");
